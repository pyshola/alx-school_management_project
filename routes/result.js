...
exports.getstudentreportcard = async function (req, res) {
    ...
    if(sch_class && session){
        ...
        if(!report_infos){
            ...
            var report_info = moment(report_infos.release_date).format('ddd MMM Do, YYYY')
        }
        ...
        var student_cls = await connect.AcademicSession.findAll({where:{class_arm_id:sch_class.id, school_id:school_id, session_id:session.id, leave:{$lte:leave}}})
        var position_score = []
        if(position == false){}
        else{
            
            for(var w = 0; w < student_cls.length; w++){
                var average_score = 0
                var average_index = 0
                var student_scores = await connect.StudentScore.findAll({where:{academic_session_id:student_cls[w].id}})
        ...
        for(var i = 0; i < student_cls.length; i++){
                    ...
                    if(isNaN(parseInt(studentscore.first_term_total))){
                        ...
                        try{
                            ...
                            total_score:'', percentage:'', position:'', grade:'', rating:''}
                                ...
                                per = per
                                for(var v = 0; v < subject_position.length; v++){
            ...
            if(req.query.term == 'Third Term'){
                var pro = await connect.StudentPromotion.findOne({where:{academic_session_id:student_cls[i].id, term:req.query.term, school_id:school_id}})
                if(pro && pro.comment == 'Promoted'){
            ...
            biodata.no_in_class = student_cls.length
            ...
            comments.generate_date = report_info
