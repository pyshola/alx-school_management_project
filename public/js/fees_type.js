$(document).ready(function(){
	...
	if(auth.isLoggedIn())
	{
		var href = 'https://alxproject.virilesoftware.com/api/feestypeadmin';
		$.ajax({
			type: 'GET',
			data: null,
			url: href,
    ...
    $(document).on('click', '.removeschoolclass', function (event) {
        ...
        if (con == true) {
            $.ajax({
                type: 'DELETE',
                data: null,
                url: 'https://alxproject.virilesoftware.com/api/feestype/' + ref_id,
                dataType: 'JSON',
                beforeSend: function (req) {
                    req.setRequestHeader('Authorization', 'Bearer ' + token);
                }
	...
	$(document).on('click', '#edit_load-submit', function (event) {
		...
		$('#updateschoolclassform').validate({
            ...
            submitHandler: function () {
                ...
                var href = 'https://alxproject.virilesoftware.com/api/feestype/'+ref_id;
                $.ajax({
                    type: 'PUT',
                    data: passd,
                    url: href,
                    ...
                    if (data.status == 'ok') {
						alert(data.msg)
						$('#row'+data.ref_id).html('<td style="padding-top:20px" colspan="4">'+ data.name + '</td>\
							  <td><div class="user-btn-wrapper">\
							<a href="#" class="btn btn-outline-warning btn-icon  editsubject" id="' +data.ref_id + '" data-name ="' + data.name + '" >\
							  <div class="tx-20"><i class="fa fa-edit"></i></div>\
							</a>\
						  <a href="#" class="btn btn-outline-danger btn-icon removesubject" id="' +data.ref_id + '" data-name ="' + data.name + '" >\
							  <div class="tx-20"><i class="fa fa-trash"></i></div>\
							</a>\
							</div></td>\
                    ')
