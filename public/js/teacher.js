$(document).ready(function(){
	...
	if(auth.isLoggedIn())
	{
		var href = 'https://alxproject.virilesoftware.com/api/teacher';
		$.ajax({
			type: 'GET',
			data: null,
			url: href,
				...
				for(i = 0; i < data.length; i++)
				{
					if(datas.delete == false){
						...
						if(data[i].status == 'Active' || data[i].status == '' || data[i].status == null)
						{
							...
							id="'+data[i].ref_id+'" data-name="'+data[i].firstname+' '+data[i].lastname+'">Disable Account</a>\
							<a class="btn btn-danger btn-sm deleteaccount" href="javascript:;"\
						 id="'+data[i].ref_id+'"  data-name="'+data[i].firstname+' '+data[i].lastname+'">Delete Account</a>'
							
						}
						else
						{
							var del = '<a class="btn btn-success btn-sm enableaccount" href="javascript:;"\
							 id="'+data[i].ref_id+'" data-name="'+data[i].firstname+' '+data[i].lastname+'">Enable Account</a>\
							<a class="btn btn-danger btn-sm deleteaccount" href="javascript:;"\
						 id="'+data[i].ref_id+'"  data-name="'+data[i].firstname+' '+data[i].lastname+'">Delete Account</a>'
						}
					}
					...
					if(datas.password == false){
						...
						var pass = '<a class="btn btn-primary btn-sm changepassword" href="javascript:;"\
							 id="'+data[i].ref_id+'"  data-name="'+data[i].firstname+' '+data[i].lastname+'">Change Password</a>'
					}
					if(datas.edit == false){
						var dis = ''
					}
					else{
						var dis = '<a class="btn btn-primary btn-sm editfish" href="https://alxproject.virilesoftware.com/eportal/'+payload.username+'/editteacher/'+data[i].ref_id+'" id="'+data[i].ref_id+'"\
							data-name="'+data[i].firstname+' '+data[i].lastname+'" data-phone="'+data[i].phone+'" >Edit</a>'
					}
					htmls +='<tr>\
                    <td><img class="img-fluid" src="'+data[i].image+'" alt="'+data[i].firstname+' '+data[i].lastname+'"\
                     style="width:48px; height:48px;border-radius:50%"/></td><td>'+data[i].username+'</td>\
    ...
    $(document).on('click', '#load-submit', function (event) {
		...
		$('#sessionform').validate({
            ...
            submitHandler: function () {
                ...
                m_data.append('username', $('#login_id').val());
				m_data.append('password', $('input[name=password]').val());
                m_data.append("passport", $("input[name=passport]")[0].files[0]);
